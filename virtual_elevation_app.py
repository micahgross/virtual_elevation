...
# cd "C:\Users\BASPO\.spyder-py3\streamlit_apps\virt_elev"
# streamlit run virtual_elevation_app.py
...
def get_raw_data(uploaded_files):
    # identify files
    laps_file = [f for f in uploaded_files if (('laps' in f.name) & ('.csv' in f.name))][0]
    starts_file = [f for f in uploaded_files if (('starts' in f.name) & ('.csv' in f.name))][0]
    ppod_file = [f for f in uploaded_files if (('Velocomp' in f.name) & ('.csv' in f.name))][0]
    power_file = [f for f in uploaded_files if (('laps' not in f.name) & ('starts' not in f.name) & ('Velocomp' not in f.name) & ('.csv' in f.name))][0]
    # read files
    df_laps = pd.read_csv(laps_file)
    df_ppod = pd.read_csv(ppod_file, header=4)
    df_power = pd.read_csv(power_file)
    
    return df_ppod, df_power, df_laps
...
def get_laps(df_power, df_laps, method='timestamp'):
    Laps = {}
    if method=='timestamp':
...
def plot_VE(Elevation, Virt_Elevation, CdA_dict, Crr_dict, display_lap):
    ...
    st.write(type(CdA_dict[display_lap]))
...
if len(uploaded_files) > 0:
    df_ppod, df_power, df_laps = get_raw_data(uploaded_files)
    df_ppod.to_json(os.path.join(os.getcwd(),'df_ppod.json'), orient='index', date_format='iso')
    df_power.to_json(os.path.join(os.getcwd(),'df_power.json'), orient='index', date_format='iso')
    df_laps.to_json(os.path.join(os.getcwd(),'df_laps.json'), orient='index', date_format='iso')
    ...
    if df_ppod is not None:
        ...
        plot_synced_speed = st.sidebar.checkbox('plot synchronized speed',
                                                value=False,
                                                key='plot_synced_speed')
        with open(os.path.join(os.getcwd(),'plot_synced_speed.json'), 'w') as fp:
        ...
        if Laps is not None:
            system_mass = st.sidebar.number_input('system mass (kg)', value=85.0, step=0.5)
            constant_elevation = st.sidebar.checkbox('assume constant elevation', value=False)
            use_wind = st.sidebar.checkbox('use wind data', value=True)
            correct_elevation_drift = st.sidebar.checkbox('correct elevation drift within laps', value=True)
            user_crr = st.sidebar.number_input('assumed Crr', value=0.005, step=0.001, format='%f')
            shift = st.sidebar.number_input('graphical shift for better alignment', value=7, step=1)
            Lap_nr_selections = {}
            ...
            display_lap = st.radio('select lap to display',
                                    options=lap_nrs)
            st.write('display_lap')
            st.write(display_lap)
            st.write(type(display_lap))
            if display_lap is not None:
                # with open(os.path.join(os.getcwd(),'lap_nrs.json'), 'w') as fp:
                #     json.dump(lap_nrs, fp)
                # with open(os.path.join(os.getcwd(),'display_lap.json'), 'w') as fp:
                #     json.dump(display_lap, fp)
                # with open(os.path.join(os.getcwd(),'system_mass.json'), 'w') as fp:
                #     json.dump(system_mass, fp)
                # with open(os.path.join(os.getcwd(),'shift.json'), 'w') as fp:
                #     json.dump(shift, fp)
                # with open(os.path.join(os.getcwd(),'constant_elevation.json'), 'w') as fp:
                #     json.dump(constant_elevation, fp)
                # with open(os.path.join(os.getcwd(),'use_wind.json'), 'w') as fp:
                #     json.dump(use_wind, fp)
                # with open(os.path.join(os.getcwd(),'correct_elevation_drift.json'), 'w') as fp:
                #     json.dump(correct_elevation_drift, fp)
                # with open(os.path.join(os.getcwd(),'user_crr.json'), 'w') as fp:
                ...
                slider_CdA = st.slider('adjust CdA manually',
                                        min_value=0.15,
                                        max_value=0.4,
                                        value=float(CdA_dict[display_lap]),
                                        step=0.001,
                                        format='%f',
                                        )
                Elevation, Virt_Elevation, CdA_dict, Crr_dict, = virtual_elevation_lap(Laps,
...
#%%
# '''
# df_ppod = pd.read_json(os.path.join(os.getcwd(),'df_ppod.json'), orient='index', convert_dates=['Date_time'])
# df_power = pd.read_json(os.path.join(os.getcwd(),'df_power.json'), orient='index', convert_dates=['Date_time'])
# df_laps = pd.read_json(os.path.join(os.getcwd(),'df_laps.json'), orient='index', convert_dates=['Date_time'])
# # with open(os.path.join(os.getcwd(),'Options.json'), 'r') as fp:
# #     Options = json.load(fp)
# with open(os.path.join(os.getcwd(),'plot_synced_speed.json'), 'r') as fp:
#     plot_synced_speed = json.load(fp)
# with open(os.path.join(os.getcwd(),'lap_nrs.json'), 'r') as fp:
#     lap_nrs = json.load(fp)
# with open(os.path.join(os.getcwd(),'display_lap.json'), 'r') as fp:
#     display_lap = json.load(fp)
# with open(os.path.join(os.getcwd(),'system_mass.json'), 'r') as fp:
#     system_mass = json.load(fp)
# with open(os.path.join(os.getcwd(),'shift.json'), 'r') as fp:
#     shift = json.load(fp)
# with open(os.path.join(os.getcwd(),'constant_elevation.json'), 'r') as fp:
#     constant_elevation = json.load(fp)
# with open(os.path.join(os.getcwd(),'use_wind.json'), 'r') as fp:
#     use_wind = json.load(fp)
# with open(os.path.join(os.getcwd(),'correct_elevation_drift.json'), 'r') as fp:
#     correct_elevation_drift = json.load(fp)
# with open(os.path.join(os.getcwd(),'user_crr.json'), 'r') as fp:
